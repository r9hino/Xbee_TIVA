...
static void BMP180Callback(void *pvCallbackData, uint_fast8_t ui8Status){
    tBMP180 *psInst;
    ...
    switch(psInst->ui8State){
        ...
        case BMP180_STATE_INIT2:
        {
            ...
            if((ui16ReadVerify == 0) || (ui16ReadVerify == 0xFFFF)){
                ...
                psInst->i16AC1 =
                    (int16_t)((psInst->uCommand.pui8Buffer[0] << 8) | psInst->uCommand.pui8Buffer[1]);
                psInst->i16AC2 =
                    (int16_t)((psInst->uCommand.pui8Buffer[2] << 8) | psInst->uCommand.pui8Buffer[3]);
                psInst->i16AC3 =
                    (int16_t)((psInst->uCommand.pui8Buffer[4] << 8) | psInst->uCommand.pui8Buffer[5]);
                psInst->ui16AC4 =
                    (uint16_t)((psInst->uCommand.pui8Buffer[6] << 8) | psInst->uCommand.pui8Buffer[7]);
                psInst->ui16AC5 =
                    (uint16_t)((psInst->uCommand.pui8Buffer[8] << 8) | psInst->uCommand.pui8Buffer[9]);
                psInst->ui16AC6 =
                    (uint16_t)((psInst->uCommand.pui8Buffer[10] << 8) | psInst->uCommand.pui8Buffer[11]);
                psInst->i16B1 =
                    (int16_t)((psInst->uCommand.pui8Buffer[12] << 8) | psInst->uCommand.pui8Buffer[13]);
                psInst->i16B2 =
                    (int16_t)((psInst->uCommand.pui8Buffer[14] << 8) | psInst->uCommand.pui8Buffer[15]);
                psInst->i16MC =
                    (int16_t)((psInst->uCommand.pui8Buffer[18] << 8) | psInst->uCommand.pui8Buffer[19]);
                psInst->i16MD =
                    (int16_t)((psInst->uCommand.pui8Buffer[20] << 8) | psInst->uCommand.pui8Buffer[21]);
        ...
        case BMP180_STATE_RMW:
        {
            ...
            if(psInst->uCommand.sReadModifyWriteState.pui8Buffer[0] == BMP180_O_CTRL_MEAS){
                // Extract the measurement mode from the CTRL_MEAS register value.
                psInst->ui8Mode =
                    (psInst->uCommand.sReadModifyWriteState.pui8Buffer[1] & BMP180_CTRL_MEAS_OSS_M);
            }
        ...
        {
            // Read the control register to see if the temperature reading is available.
        ...
        case BMP180_STATE_WAIT_TEMP:
        {
            ...
            if(psInst->uCommand.pui8Buffer[1] & BMP180_CTRL_MEAS_SCO){
                ...
                I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer, 1,
                         psInst->uCommand.pui8Buffer + 1, 1, BMP180Callback, psInst);
            }
            else{
                ...
                I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer,
                		 1, psInst->pui8Data, 2, BMP180Callback, psInst);
        ...
        case BMP180_STATE_READ_TEMP:
        {
            ...
            psInst->uCommand.pui8Buffer[1] = (BMP180_CTRL_MEAS_SCO |
                                              BMP180_CTRL_MEAS_PRESSURE | psInst->ui8Mode);
        ...
        {
            // Read the control register to see if the pressure reading is available.
            I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer, 1,
                     psInst->uCommand.pui8Buffer + 1, 1, BMP180Callback, psInst);
        ...
        case BMP180_STATE_WAIT_PRES:
        {
            ...
            if(psInst->uCommand.pui8Buffer[1] & BMP180_CTRL_MEAS_SCO){
                // The pressure reading is not ready yet, so read the control register again.
                I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer, 1,
                         psInst->uCommand.pui8Buffer + 1, 1, BMP180Callback, psInst);
            }
            else{
                ...
                psInst->uCommand.pui8Buffer[0] = BMP180_O_OUT_MSB;
                I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer,
                		 1, psInst->pui8Data + 2, 3, BMP180Callback, psInst);
...
//
//*****************************************************************************
uint_fast8_t BMP180Init(tBMP180 *psInst, tI2CMInstance *psI2CInst, uint_fast8_t ui8I2CAddr,
           tSensorCallback *pfnCallback, void *pvCallbackData)
{
    ...
    if(I2CMWrite(psI2CInst, ui8I2CAddr, psInst->pui8Data, 2, BMP180Callback, psInst) == 0){
...
//
//*****************************************************************************
uint_fast8_t BMP180Read(tBMP180 *psInst, uint_fast8_t ui8Reg, uint8_t *pui8Data,
           uint_fast16_t ui16Count, tSensorCallback *pfnCallback, void *pvCallbackData)
{
    ...
    if(I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer,
    			1, pui8Data, ui16Count, BMP180Callback, psInst) == 0){
        // The I2C write failed, so move to the idle state and return a failure.
        psInst->ui8State = BMP180_STATE_IDLE;
        return(0);
    }
...
//
//*****************************************************************************
uint_fast8_t BMP180Write(tBMP180 *psInst, uint_fast8_t ui8Reg, uint8_t *pui8Data,
            uint_fast16_t ui16Count, tSensorCallback *pfnCallback, void *pvCallbackData)
{
    // Return a failure if the BMP180 driver is not idle (in other words, there
    // is already an outstanding request to the BMP180).
    ...
    if((ui8Reg <= BMP180_O_CTRL_MEAS) &&
       ((ui8Reg + ui16Count) > BMP180_O_CTRL_MEAS)){
        // Extract the measurement mode from the CTRL_MEAS register value.
        psInst->ui8NewMode = (pui8Data[ui8Reg - BMP180_O_CTRL_MEAS] & BMP180_CTRL_MEAS_OSS_M);
    }
    ...
    if(I2CMWrite8(&(psInst->uCommand.sWriteState), psInst->psI2CInst,
                  psInst->ui8Addr, ui8Reg, pui8Data, ui16Count, BMP180Callback, psInst) == 0)
    {
        // The I2C write failed, so move to the idle state and return a failure.
        psInst->ui8State = BMP180_STATE_IDLE;
        return(0);
...
//
//*****************************************************************************
uint_fast8_t BMP180ReadModifyWrite(tBMP180 *psInst, uint_fast8_t ui8Reg,
                      ...
                      tSensorCallback *pfnCallback, void *pvCallbackData)
{
    ...
    {
        // The I2C read-modify-write failed, so move to the idle state and return a failure.
        psInst->ui8State = BMP180_STATE_IDLE;
        return(0);
    }
...
//
//*****************************************************************************
uint_fast8_t BMP180DataRead(tBMP180 *psInst, tSensorCallback *pfnCallback, void *pvCallbackData){
    // Return a failure if the BMP180 driver is not idle (in other words, there
    // is already an outstanding request to the BMP180).
    if(psInst->ui8State != BMP180_STATE_IDLE){
        return(0);
    ...
    psInst->uCommand.pui8Buffer[0] = BMP180_O_CTRL_MEAS;
    psInst->uCommand.pui8Buffer[1] = (BMP180_CTRL_MEAS_SCO | BMP180_CTRL_MEAS_TEMPERATURE);
    if(I2CMWrite(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer, 2, BMP180Callback, psInst) == 0){
        // The I2C write failed, so move to the idle state and return a failure.
        psInst->ui8State = BMP180_STATE_IDLE;
        return(0);
    }
...
//
//*****************************************************************************
void BMP180DataPressureGetRaw(tBMP180 *psInst, uint_fast32_t *pui32Pressure){
    // Return the raw pressure value.
    *pui32Pressure = ((psInst->pui8Data[2] << 16) | (psInst->pui8Data[3] << 8) |
...
void BMP180DataPressureGetFloat(tBMP180 *psInst, float *pfPressure){
    ...
    fUT = (float)(uint16_t)((psInst->pui8Data[0] << 8) | psInst->pui8Data[1]);
    fUP = ((float)(int32_t)((psInst->pui8Data[2] << 16) | (psInst->pui8Data[3] << 8) |
                            (psInst->pui8Data[4] & BMP180_OUT_XLSB_M)) / (1 << (8 - i8Oss)));
...
//
//*****************************************************************************
void BMP180DataTemperatureGetRaw(tBMP180 *psInst, uint_fast16_t *pui16Temperature){
