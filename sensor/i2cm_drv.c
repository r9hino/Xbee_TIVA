...
static void I2CMStateIdle(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    ...
    if(pCommand->ui16WriteCount != 0){
        ...
        // See if there is just a single byte to be written and no bytes to be read.
        ...
        // Otherwise, see if there is just a single byte to be written and at least one byte to be read.
        else if(pCommand->ui16WriteCount == 1){
            // Perform a single send, writing the first byte as the only byte.
            MAP_I2CMasterControl(psInst->ui32Base, I2C_MASTER_CMD_BURST_SEND_START);
        ...
        // Otherwise, there is more than one byte to be written.
            ...
            // Set the next state of the interrupt state machine based on the number of bytes to write.
        ...
        if(pCommand->ui16ReadCount == 1){
            ...
            // Set the next state appropriately. If the read count is two, the next state must finish
            // the transaction. If it is greater than two, the burst read must be continued.
...
static void I2CMStateWriteNext(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    ...
    if(psInst->ui16Index == pCommand->ui16WriteBatchSize){
        ...
        MAP_I2CMasterDataPut(psInst->ui32Base, pCommand->pui8WriteData[psInst->ui16Index]);
        psInst->ui16Index++;
...
static void I2CMStateWriteFinal(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    // See if the write batch has been sent.
    if(psInst->ui16Index == pCommand->ui16WriteBatchSize){
        // Move to the write pause state.
        psInst->ui8State = STATE_WRITE_PAUSE;
        ...
        if(pCommand->ui16ReadCount == 0){
            ...
            MAP_I2CMasterControl(psInst->ui32Base, I2C_MASTER_CMD_BURST_SEND_FINISH);
...
static void I2CMStateWritePause(tI2CMInstance *psInst, tI2CMCommand *pCommand){
...
static void I2CMStateReadOne(tI2CMInstance *psInst, tI2CMCommand *pCommand){
...
static void I2CMStateReadFirst(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    // Put the I2C master into receive mode.
    MAP_I2CMasterSlaveAddrSet(psInst->ui32Base, pCommand->ui8Addr, true);
...
static void I2CMStateReadNext(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    // Read the received character.
    pCommand->pui8ReadData[psInst->ui16Index] = MAP_I2CMasterDataGet(psInst->ui32Base);
    psInst->ui16Index++;
    ...
    if(psInst->ui16Index == pCommand->ui16ReadBatchSize){
        ...
        MAP_I2CMasterControl(psInst->ui32Base, I2C_MASTER_CMD_BURST_RECEIVE_CONT);
...
static void I2CMStateReadFinal(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    // Read the received character.
    pCommand->pui8ReadData[psInst->ui16Index] = MAP_I2CMasterDataGet(psInst->ui32Base);
    psInst->ui16Index++;
    ...
    if(psInst->ui16Index == pCommand->ui16ReadBatchSize){
        ...
        MAP_I2CMasterControl(psInst->ui32Base, I2C_MASTER_CMD_BURST_RECEIVE_FINISH);
...
static void I2CMStateReadPause(tI2CMInstance *psInst, tI2CMCommand *pCommand){
...
static void I2CMStateReadWait(tI2CMInstance *psInst, tI2CMCommand *pCommand){
    // Read the received character.
    pCommand->pui8ReadData[psInst->ui16Index] = MAP_I2CMasterDataGet(psInst->ui32Base);
...
static void I2CMStateCallback(tI2CMInstance *psInst, tI2CMCommand *pCommand, uint32_t ui32Status){
    tSensorCallback *pfnCallback;
    void *pvCallbackData;
    ...
    if(pfnCallback){
        // Convert the status from the I2C driver into the I2C master driver status.
...
void I2CMIntHandler(tI2CMInstance *psInst){
    ...
    if((ui32Status & (I2C_MCS_ERROR | I2C_MCS_ARBLST)) && (psInst->ui8State != STATE_IDLE)){
        ...
        if(ui32Status & I2C_MCS_BUSBSY){
            MAP_I2CMasterControl(psInst->ui32Base, I2C_MASTER_CMD_BURST_SEND_ERROR_STOP);
        }
    ...
    while(1){
        ...
        switch(psInst->ui8State){
            ...
            case STATE_CALLBACK:
            {
                ...
                // If an error occurred, this state is done. The completion of the error handling
                // stop condition above, if issued, will cause the next state to be processed.
...
//
//*****************************************************************************
void I2CMInit(tI2CMInstance *psInst, uint32_t ui32Base, uint_fast8_t ui8Int,
...
//
//*****************************************************************************
uint_fast8_t I2CMCommand(tI2CMInstance *psInst, uint_fast8_t ui8Addr,
...
//
//*****************************************************************************
uint_fast8_t I2CMTransferResume(tI2CMInstance *psInst, uint8_t *pui8Data){
    // Check the arguments.
    ASSERT(psInst);
    ASSERT(pui8Data);

    // Return an error if there is not a paused transfer.
    if((psInst->ui8State != STATE_WRITE_PAUSE) && (psInst->ui8State != STATE_READ_PAUSE)){
        return(0);
    }
...
static void I2CMReadModifyWrite8Callback(void *pvCallbackData, uint_fast8_t ui8Status){
    tI2CMReadModifyWrite8 *psInst;

    // Convert the instance data into a pointer to a tI2CMReadModifyWrite8 structure.
    psInst = pvCallbackData;

    // If the I2C master driver encountered a failure, force the state machine
    // to the idle state (which will also result ina  callback to propagate the error).
    if(ui8Status != I2CM_STATUS_SUCCESS){
        psInst->ui8State = I2CM_RMW_STATE_IDLE;
    }

    // Determine the current state of the I2C master read-modify-write state machine.
    switch(psInst->ui8State){
        ...
        case I2CM_RMW_STATE_READ:
        {
            ...
            psInst->pui8Buffer[1] = ((psInst->pui8Buffer[1] & psInst->ui8Mask) | psInst->ui8Value);

            // Write the data back to the device.
            I2CMWrite(psInst->psI2CInst, psInst->ui8Addr, psInst->pui8Buffer, 2, I2CMReadModifyWrite8Callback, psInst);
...
//
//*****************************************************************************
uint_fast8_t I2CMReadModifyWrite8(tI2CMReadModifyWrite8 *psInst, tI2CMInstance *psI2CInst,
    ...
    // Fill in the read-modify-write structure with the details of this request.
    ...
    if(ui8Mask == 0){
        // Set the state to waiting for the write portion of the read-modify-write.
        psInst->ui8State = I2CM_RMW_STATE_WRITE;
        ...
        if(I2CMWrite(psI2CInst, ui8Addr, psInst->pui8Buffer, 2, I2CMReadModifyWrite8Callback, psInst) == 0){
            return(0);
        }
    }
    else{
        // Set the state to waiting for the read portion of the read-modify-write.
        psInst->ui8State = I2CM_RMW_STATE_READ;

        // Add the read command to the I2C master queue.
        if(I2CMRead(psI2CInst, ui8Addr, psInst->pui8Buffer, 1, psInst->pui8Buffer + 1,
        		1, I2CMReadModifyWrite8Callback, psInst) == 0){
            return(0);
        }
    }
...
static void I2CMReadModifyWrite16LECallback(void *pvCallbackData, uint_fast8_t ui8Status){
    tI2CMReadModifyWrite16 *psInst;
    uint16_t ui16Value;

    // Convert the instance data into a pointer to a tI2CMReadModifyWrite16 structure.
    psInst = pvCallbackData;
    ...
    switch(psInst->ui8State){
        ...
        case I2CM_RMW_STATE_READ:
        {
            ...
            // Write the data back to the device.
            I2CMWrite(psInst->psI2CInst, psInst->ui8Addr, psInst->pui8Buffer, 3, I2CMReadModifyWrite16LECallback, psInst);
...
//
//*****************************************************************************
uint_fast8_t I2CMReadModifyWrite16LE(tI2CMReadModifyWrite16 *psInst,
    ...
    if(ui16Mask == 0){
        ...
        // Add the write command to the I2C master queue.
        if(I2CMWrite(psI2CInst, ui8Addr, psInst->pui8Buffer, 3, I2CMReadModifyWrite16LECallback, psInst) == 0){
            return(0);
        }
    }
    else{
        ...
        if(I2CMRead(psI2CInst, ui8Addr, psInst->pui8Buffer, 1,
                    psInst->pui8Buffer + 1, 2, I2CMReadModifyWrite16LECallback, psInst) == 0){
            return(0);
        }
    }
...
static void I2CMWrite8Callback(void *pvCallbackData, uint_fast8_t ui8Status){
    tI2CMWrite8 *psInst;
    ...
    // The transfer has completed, or an error has occurred.  In both cases, see if there is a callback function.
...
//
//*****************************************************************************
uint_fast8_t I2CMWrite8(tI2CMWrite8 *psInst, tI2CMInstance *psI2CInst, uint_fast8_t ui8Addr,
...
static void I2CMRead16BECallback(void *pvCallbackData, uint_fast8_t ui8Status){
...
//
//*****************************************************************************
uint_fast8_t I2CMRead16BE(tI2CMRead16BE *psInst, tI2CMInstance *psI2CInst,
             uint_fast8_t ui8Addr, uint_fast8_t ui8Reg,
...
static void I2CMWrite16BECallback(void *pvCallbackData, uint_fast8_t ui8Status){
...
//
//*****************************************************************************
uint_fast8_t I2CMWrite16BE(tI2CMWrite16BE *psInst, tI2CMInstance *psI2CInst,
              uint_fast8_t ui8Addr, uint_fast8_t ui8Reg,
    ...
    if(I2CMWriteBatched(psI2CInst, ui8Addr, psInst->pui8Buffer,
                        (ui16Count * 2) + 1, 2, I2CMWrite16BECallback, psInst) == 0){
        // The I2C write failed, so return a failure.
        return(0);
    }
