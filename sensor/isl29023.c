...
const float g_fRangeLookup[4] = {1000.0, 4000.0, 16000.0, 64000.0};

//*****************************************************************************
// Resolution setting to floating point resolution value lookup table
const float g_fResolutionLookup[4] = {65536.0, 4096.0, 256.0, 16.0};
...
const float g_fBetaLookup[4] = {95.238, 23.810, 5.952, 1.486};
...
static void ISL29023Callback(void *pvCallbackData, uint_fast8_t ui8Status){
    tISL29023 *psInst;
    uint8_t *pui8Data;
    ...
    switch(psInst->ui8State){
        ...
        case ISL29023_STATE_WRITE:
        {
            ...
            // was not modified, the values will be the same so this has no effect.
        ...
        case ISL29023_STATE_RMW:
        {
            ...
            if((pui8Data[0] == ISL29023_O_CMD_II) && (ui8Status == I2CM_STATUS_SUCCESS)){
                // Store the latest range and resolution settings
                psInst->ui8Range = ((pui8Data[1] & ISL29023_CMD_II_RANGE_M) >> ISL29023_CMD_II_RANGE_S);
...
//
//*****************************************************************************
uint_fast8_t ISL29023Init(tISL29023 *psInst, tI2CMInstance *psI2CInst,
             uint_fast8_t ui8I2CAddr, tSensorCallback *pfnCallback, void *pvCallbackData){
    ...
    psInst->ui8Resolution = (ISL29023_CMD_II_ADC_RES_16 >> ISL29023_CMD_II_ADC_RES_S);
    psInst->ui8NewResolution = (ISL29023_CMD_II_ADC_RES_16 >> ISL29023_CMD_II_ADC_RES_S);
    ...
    if(I2CMWrite(psInst->psI2CInst, psInst->ui8Addr, psInst->pui8Data, 2, ISL29023Callback, psInst) == 0){
...
//
//*****************************************************************************
uint_fast8_t ISL29023ReadModifyWrite(tISL29023 *psInst, uint_fast8_t ui8Reg,
...
//*****************************************************************************
uint_fast8_t
ISL29023DataRead(tISL29023 *psInst, tSensorCallback *pfnCallback, void *pvCallbackData){
    // Return a failure if the ISL29023 driver is not idle (in other words,
    // there is already an outstanding request to the ISL29023).
    if(psInst->ui8State != ISL29023_STATE_IDLE){
        return(0);
    ...
    if(I2CMRead(psInst->psI2CInst, psInst->ui8Addr, psInst->uCommand.pui8Buffer,
    		    1, psInst->pui8Data, 2, ISL29023Callback, psInst) == 0){
...
//
//*****************************************************************************
void ISL29023DataLightVisibleGetRaw(tISL29023 *psInst, uint16_t *pui16Visible){
...
//
//*****************************************************************************
void ISL29023DataLightIRGetRaw(tISL29023 *psInst, uint16_t *pui16IR){
    // Return the raw light value.
