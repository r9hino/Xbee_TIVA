...
static void SHT21Callback(void *pvCallbackData, uint_fast8_t ui8Status){
    tSHT21 *psInst;

    // Convert the instance data into a pointer to a tSHT21 structure.
...
//
//*****************************************************************************
uint_fast8_t SHT21Init(tSHT21 *psInst, tI2CMInstance *psI2CInst, uint_fast8_t ui8I2CAddr,
    ...
    if(I2CMWrite(psInst->psI2CInst, ui8I2CAddr, psInst->pui8Data, 1, SHT21Callback, psInst) == 0){
...
//
//*****************************************************************************
uint_fast8_t SHT21Read(tSHT21 *psInst, uint_fast8_t ui8Reg, uint8_t *pui8Data,
...
//
//*****************************************************************************
uint_fast8_t  SHT21Write(tSHT21 *psInst, uint_fast8_t ui8Reg, const uint8_t *pui8Data,
           uint_fast16_t ui16Count, tSensorCallback *pfnCallback, void *pvCallbackData)
{
    // Return a failure if the SHT21 driver is not idle (in other words, there
    // is already an outstanding request to the SHT21).
    ...
    if(I2CMWrite8(&(psInst->uCommand.sWriteState), psInst->psI2CInst, psInst->ui8Addr,
    		ui8Reg, pui8Data, ui16Count, SHT21Callback, psInst) == 0){
        // The I2C write failed, so move to the idle state and return a failure.
        psInst->ui8State = SHT21_STATE_IDLE;
        return(0);
    }
...
//
//*****************************************************************************
uint_fast8_t SHT21ReadModifyWrite(tSHT21 *psInst, uint_fast8_t ui8Reg, uint_fast8_t ui8Mask,
                     uint_fast8_t ui8Value, tSensorCallback *pfnCallback, void *pvCallbackData){
    // Return a failure if the SHT21 driver is not idle (in other words, there
    // is already an outstanding request to the SHT21).
    if(psInst->ui8State != SHT21_STATE_IDLE){
        return(0);
...
//! read has completed (as indicated by callback function), the new
...
//
//*****************************************************************************
uint_fast8_t SHT21DataRead(tSHT21 *psInst, tSensorCallback *pfnCallback, void *pvCallbackData){
    // Return a failure if the SHT21 driver is not idle (in other words, there
    // is already an outstanding request to the SHT21).
    if(psInst->ui8State != SHT21_STATE_IDLE){
        return(0);
...
//
//*****************************************************************************
void SHT21DataTemperatureGetRaw(tSHT21 *psInst, uint16_t *pui16Temperature){
...
//
//*****************************************************************************
void SHT21DataHumidityGetRaw(tSHT21 *psInst, uint16_t *pui16Humidity){
    // Return the raw humidity value.
    *pui16Humidity = ((((uint16_t)psInst->pui8Data[0]) << 8) | (uint16_t)psInst->pui8Data[1]);
}
